...
def login():
    ...
    if users.find_one({"username": data['username'], "password": data['password']}):
        user_uuid = create_session({"username": data['username']})
...
def get_student():
    ...
    if is_session_valid(uuid):
        studentemail = students.find_one({"email":data['email']})
        student = json.loads(json_util.dumps(studentemail))
        ...
        if (students.find({"email":data['email']}).count() == 1):
             ...
             return Response("Error:No Student was found!!!!", status=400, mimetype="application/json")
    else:
        return Response("There is not a student with this email(student has not been authenticated).", status=401, mimetype='application/json')
...
def get_students_thirty():
    ...
    if is_session_valid(uuid):
        ...
        studentage = students.find({"$and": [{ "yearOfBirth": { "$eq": 1991 }}]})
        if (students.find({"$and": [{ "yearOfBirth": { "$eq": 1991 }}]}).count() == 0):
            return Response("There is not a student with that age!!!", status=400, mimetype="application/json")
        else:
            studentarr = []
            student = json.loads(json_util.dumps(studentage))

            for j in student:
                j['_id'] = None # This is used in order to make the json object serialized, otherwise this will result in error!!
                studentarr.append(j)

            return Response(json.dumps(studentarr), status=200, mimetype='application/json')
    else:
        return Response("There is not a student with is 30 years old(student has not been authenticated).", status=401, mimetype='application/json')
...
def get_students_oldies():
    ...
    if is_session_valid(uuid):
        ...
        studentage = students.find({"$and": [{ "yearOfBirth": { "$lte": 1991 }}]})
        if (students.find({"$and": [{ "yearOfBirth": { "$lte": 1991 }}]}).count() == 0):
            return Response("There is not a student with that age!!!", status=400, mimetype="application/json")
        else:
            studentarr = []
            student = json.loads(json_util.dumps(studentage))
            ...
            for k in student:
                k['_id'] = None # This is used in order to make the json object serialized, otherwise this will result in error!!
                studentarr.append(k)

            return Response(json.dumps(student), status=200, mimetype='application/json')
    else:
        return Response("There is not a student with is older than 30 years old(student has not been authenticated).", status=401, mimetype='application/json')
    """
...
def get_student_address():
    ...
    if is_session_valid(uuid):
        studentemail = students.find_one({"email":data['email']})
        stud = json.loads(json_util.dumps(studentemail))
        # studentadd = students.find_one({"name":data['name'], json_util.loads({"name":data['name']}),"postcode":data['address']['street']['city']['postcode']})
        
        if (students.find({"email":data['email']}).count() == 1):
            
            student = {}
            student['name'] = stud['name']
            student['street'] = stud['address'][0]['street']
            student['postcode'] = stud['address'][0]['postcode']
            ...
            return Response(json.dumps(student), status=200, mimetype='application/json')
        elif (students.find({"address":data['address']}).count() == 0) or (students.find({"email":data['email']}).count() == 0):
            return Response("The student does not exist or doesn't have their address reported!!", status=400, mimetype="application/json")
    else:
        return Response("(Student has not been authenticated).", status=401, mimetype='application/json')
...
def delete_student():
    ...
    if is_session_valid(uuid):
        ...
        if (students.find({"email":data['email']}).count() == 1):
            ...
            return Response(student['name'] + "has been deleted!!!!", status=200, mimetype='application/json')
        else:
            return Response("Error:No Student was found!!!!", status=400, mimetype="application/json")
    else:
        return Response("There is not a student with this email(student has not been authenticated).", status=401, mimetype='application/json')
...
def add_courses():
    ...
    if is_session_valid(uuid):
        ...
        if (students.find({"email":data['email']}).count() == 1):
            res = students.update_one({"email":data['email']}, {'$set' : { "courses" : data['courses']}})
            # Η παρακάτω εντολή χρησιμοποιείται μόνο στη περίπτωση επιτυχούς αναζήτησης φοιτητών (δηλ. υπάρχει φοιτητής με αυτό το email).
            return Response("The user" + student['name'] + "was updated", status=200, mimetype='application/json')
        else:
            return Response("Error:No Student was found!!!!", status=400, mimetype="application/json")
    else:
        return Response("There is not a student with this email(student has not been authenticated).", status=401, mimetype='application/json')
...
def get_courses():
    ...
    if is_session_valid(uuid):
        ...
        if (students.find({"email":data['email']}).count() == 1):
            res = {}
            studentarr = student['courses']
            for i in range(len(studentarr)):
                res.update(studentarr[i])
            ...
            # Η παρακάτω εντολή χρησιμοποιείται μόνο στη περίπτωση επιτυχούς αναζήτησης φοιτητών (δηλ. υπάρχει φοιτητής με αυτό το email).
            return Response(json.dumps(kolos), status=200, mimetype='application/json')
        elif (students.find({"email": data['email']}).count() == 0) or (students.find({"courses": data['courses']}).count() == 0):
            return Response("Error:No Student was found!!!!", status=400, mimetype="application/json")
    else:
        return Response("There is not a student with this email(student has not been authenticated).", status=401, mimetype='application/json')
